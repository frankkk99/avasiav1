import { buildUpstreamHeaders, defaultUserAgent, validateUpstreamUrl } from "@/lib/security";

export type HlsDiagnosticInput = {
  url: string;
  origin?: string;
  referer?: string;
  userAgent?: string;
...
export async function diagnoseHlsManifest(input: HlsDiagnosticInput) {
  const url = validateUpstreamUrl(input.url);
  ...
  if (input.testSegment !== false && response.ok && isHls) {
  ...
  return {
    ok: response.ok && isHls && segmentTest?.ok !== false,
    manifest: {
    ...
    segmentTest,
    proxyEnabled: process.env.ENABLE_STREAM_PROXY === "true",
    serverNote:
